...
fn main() -> Result<()> {
    ...
    println!(
        "   Total global data symbols: {}",
        parsed_pdb.global_data.len()
    );
    ...
    for data_symbol in parsed_pdb.global_data.iter() {
        ...
        } else if type_name.contains("Primitive")
    ...
    if !ipi_data_symbols.is_empty() {
        ...
        println!(
            "\n⚠️  {:.1}% of data symbols have IPI type indices!",
            percentage
        );
    }
    ...
    println!(
        "\nUnique function names in FuncId: {}",
        unique_funcnames.len()
    );
