use anyhow::Result;
use ms_pdb::{codeview::types::TypeIndex, Pdb};
use std::fs::File;
...
fn main() -> Result<()> {
    ...
    println!("═══════════════════════════════════════════════════════════════\n");
    println!(
        "Data symbols with type in TPI only (CORRECT): {}",
        symbols_tpi_only.len()
    );
    println!(
        "Data symbols with type in IPI only (BUG): {}",
        symbols_ipi_only.len()
    );
    println!(
        "Data symbols with type in BOTH TPI and IPI (COLLISION): {}",
        symbols_with_collisions.len()
    );
            ...
            if let Ok(ipi_record) = ipi_stream.record(*type_idx) {
                if let Ok(ipi_data) = ipi_record.parse() {
                    let type_name = match ipi_data {
                        ms_pdb::codeview::types::TypeData::FuncId(ref f) => {
                            format!("FuncId({})", f.name)
                        }
                        ms_pdb::codeview::types::TypeData::UdtSrcLine(ref u) => {
                            format!("UdtSrcLine(line {})", u.line.get())
                        }
                        _ => format!("{:?}", ipi_data),
                    };
