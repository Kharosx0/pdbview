...
fn main() -> Result<()> {
    ...
    for symbol_name in suspicious_symbols {
        ...
        // Find in OLD pdb
        let old_symbol = old_pdb.global_data.iter().find(|s| s.name == symbol_name);

        // Find in NEW pdb
        let new_symbol = new_pdb.global_data.iter().find(|s| s.name == symbol_name);
    ...
    println!(
        "OLD pdb crate: {} data symbols with IPI types",
        old_ipi_count
    );
    println!(
        "NEW ms-pdb:    {} data symbols with IPI types",
        new_ipi_count
    );
    ...
    if old_ipi_count == new_ipi_count && new_ipi_count > 0 {
        ...
        println!(
            "\n🚨 OLD parser has ZERO IPI types, NEW has {}!",
            new_ipi_count
        );
        ...
        println!(
            "   Difference: {} symbols",
            (old_ipi_count as i32 - new_ipi_count as i32).abs()
        );
    }
