...
fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Some(ipi_type) = parsed_pdb.ipi_types.get(idx) {
            ...
            if tpi_discriminant == ipi_discriminant {
                ...
                if collision_count <= 5 {
                    ...
                    println!(
                        "  TPI: {:?}",
                        format!("{:?}", &*tpi_borrowed).lines().next().unwrap_or("")
                    );
                    println!(
                        "  IPI: {:?}",
                        format!("{:?}", &*ipi_borrowed).lines().next().unwrap_or("")
                    );
                    println!();
                }
            } else {
                ...
                if different_type_count <= 10 {
                    ...
                    match &*ipi_borrowed {
                        ...
                        Type::UdtSrcLineType(_) => {
                            println!("  → IPI is UdtSrcLine (IPI-only type)")
                        }
    ...
    println!(
        "Same type variant: {} ({:.1}%)",
        same_type_count,
        (same_type_count as f64 / collision_count as f64) * 100.0
    );
    println!(
        "Different type variant: {} ({:.1}%)",
        different_type_count,
        (different_type_count as f64 / collision_count as f64) * 100.0
    );

    if different_type_count > 0 {
        println!(
            "\n⚠️ Found {} collisions with DIFFERENT types - this explains the problem!",
            different_type_count
        );
        println!(
            "We need to determine which stream based on the record type, not just check TPI first!"
        );
    }
